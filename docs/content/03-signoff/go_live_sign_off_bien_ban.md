# BIEN BAN GO-LIVE SIGN-OFF - Enterprise Chatbot | AI

## 1. Thong tin chung

- Ten du an: `demo-chatbot`
- So hop dong:
- Khach hang (Ben A):
- Don vi cung cap (Ben B):
- Ngay danh gia go-live:
- Ngay lap bien ban:

## 2. Pham vi danh gia go-live

- [ ] Hoan tat UAT sign-off
- [ ] Hoan tat checklist go-live
- [ ] San sang van hanh production
- [ ] Tai lieu tham chieu: [Go-live checklist](../02-governance/go_live_checklist.md), [SLA/SLO](../02-governance/slo_sla.md), [Runbook incident](../02-governance/runbook_incident.md), [Risk register](../02-governance/risk_register.md)

## 3. Kiem tra dieu kien truoc go-live

- [ ] Khong con loi Sev1
- [ ] Loi Sev2 con lai co workaround va deadline fix
- [ ] KPI hieu nang dat nguong thong nhat
- [ ] Co rollback plan ro rang
- [ ] Co on-call va kenh incident da san sang
- [ ] Da xac nhan monitoring/alerting

## 4. Ket qua danh gia ky thuat

- Uptime baseline:
- p95 latency:
- 5xx error rate:
- Ti le timeout:
- Trang thai logging/monitoring:

## 5. Van de ton dong truoc go-live (neu co)

| ID | Mo ta | Muc do | Owner | Han xu ly | Anh huong go-live |
|---|---|---|---|---|---|
|  |  |  |  |  |  |

## 6. Ket luan Go-live Sign-off

Chon 1:
- [ ] **GO** - Du dieu kien dua vao production
- [ ] **GO CO DIEU KIEN** - Cho phep go-live kem rang buoc xu ly ton dong
- [ ] **NO-GO** - Chua du dieu kien, lui lich go-live

Neu GO CO DIEU KIEN, liet ke rang buoc:
1.
2.
3.

## 7. Ke hoach sau go-live

- Hypercare:
  - [ ] 3 ngay
  - [ ] 7 ngay
  - [ ] 14 ngay
- Lich review sau go-live:
- Dau moi xu ly su co:

## 8. Xac nhan cua cac ben

### Ben A (Khach hang)

- Dai dien:
- Chuc vu:
- Chu ky:
- Ngay ky:

### Ben B (Don vi cung cap)

- Dai dien:
- Chuc vu:
- Chu ky:
- Ngay ky:

### Dai dien Ops/Security (neu co)

- Dai dien Ops:
- Dai dien Security:

---

## Noi dung xac nhan ngan (mau)

"Hai ben xac nhan he thong da [du/chua du] dieu kien go-live theo checklist va KPI da thong nhat. Truong hop GO/GO CO DIEU KIEN, he thong duoc phep dua vao production theo ke hoach va co che giam sat sau go-live."
